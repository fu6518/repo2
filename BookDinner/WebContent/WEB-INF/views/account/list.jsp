...
<script type="text/javascript">
	...
	*/
	function add(){
		...
		if(!validate){
			layer.msg("消息提醒","请检查你输入的数据", {time:700, icon:3, shift:2}, function(){/* 显示框的时间，笑脸表情，抖动效果 */
		...
		$.ajax({
			...
			success:function(data){
				if(data.type == 'success'){
					layer.msg('信息提示','添加成功！', {time:700, icon:2, shift:2}, function(){/* 显示框的时间，笑脸表情，抖动效果 */
	...
	*/
	function edit(){
		...
		$.ajax({
			...
			success:function(data){
				if(data.type == 'success'){
					layer.msg('信息提示','修改成功！', {time:700, icon:2, shift:2}, function(){/* 显示框的时间，笑脸表情，抖动效果 */
	...
	*/
	function remove(){
		 layer.confirm("删除用户信息【"+name+"】, 是否继续",  {icon: 3, title:'提示'}, function(result){
			if(result){
				...
				$.ajax({
					...
					success:function(data){
						if(data.type == 'success'){
					layer.msg("用户信息删除成功", {time:2000, icon:5, shift:6}, function(){
	...
	*/
	function openEdit(){
		...
		if(item == null || item.length == 0){
			layer.msg('信息提示','请选择要修改的数据！', {time:2000, icon:5, shift:6}, function(){
            	
            });
			return;
		}
		if(item.length > 1){
			layer.msg('信息提示','请选择一条数据进行修改！', {time:2000, icon:5, shift:6}, function(){
